...
class KalmanFilter(object):
	...
	def __init__(self, q, r, p=np.float32(0.0), k=np.float32(0.0), initial_value=np.float32(initialTemp)):
		self.q = q
		self.r = r
		self.p = p
...
plt.text(10, 32, r'r='+str(rFilt) +', q='+str(qFilt)+' stanDev= '+str(np.std(diffArray)))
plt.grid(True)
plt.legend( (inputPlot, outputPlot),
           ('Input Angle', 'Filtered Angle'),
           'lower right' )
plt.show()
